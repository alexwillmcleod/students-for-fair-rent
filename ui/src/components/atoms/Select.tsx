...
export default function Select<T extends string>(props: SelectProps<T>) {
  ...
  return (
    <div class="flex flex-col justify-center items-center relative">
      <button
        class="flex flex-row gap-4 justify-center items-center md:w-64 lg:w-full text-ellipsis shadow-xl rounded-xl p-4 bg-white/40"
        onClick={() => setIsOpen(!isOpen())}
      >
        {props.value || 'Select an option'}
        <img
          class="w-4"
          src="/images/down-dropdown.svg"
        />
      </button>
      <Show when={isOpen()}>
        <ul
          class="transition-all bg-white/50 backdrop-blur-lg shadow-xl rounded-xl flex flex-col justify-center items-center absolute top-full"
          style={dropdownStyle}
        >
